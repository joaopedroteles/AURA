...
const Promo = () => {
  const [university, setUniversidade] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchBanners = async () => {
      try {
        ...
        const filtered = res.data.filter((b) => b.location === 'universidade');
        setBanners(filtered);
      } catch (error) {
    ...
    fetchBanners();
    fetchProdutosPorArea('university', setUniversidade);
  }, []);
...
<Carousel autoPlay interval={3000} infiniteLoop showThumbs={false} showStatus={false}>
      ...
      <h1 className="home-title">Camisetas de Universidade</h1>
      {renderProdutos(university)}
    </div>
  );
};
